...
export function legalActions(board: BoardState, player: Player, movesThisTurn = 0): Action[] {
    ...
    for (let col = 0; col < board.config.cols; col += 1) {
      ...
      for (let dr = -2; dr <= 2; dr += 1) {
        for (let dc = -2; dc <= 2; dc += 1) {
          ...
          const action = buildAction(board, [row, col], [row + dr, col + dc], { movesThisTurn });
          if (!action.error) {
            actions.push(action);
          }
        }
...
export function chooseDeterministicAction(board: BoardState, player: Player, movesThisTurn = 0): Action {
  const actions = legalActions(board, player, movesThisTurn);
