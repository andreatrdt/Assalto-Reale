...
import pygame
import asyncio
IS_WEB = sys.platform in ("emscripten", "wasi")  # pygbag targets
...
class GameConfig:
    ...
    ASSETS_DIR: str = field(
        default_factory=lambda: ("assets" if IS_WEB else os.path.join(os.path.dirname(__file__), "assets"))
    )
...
class AssetLoader:
    """Loads sprites immediately; defers audio init until user click (browser-safe)."""
    ...
    def enable_audio(self) -> None:
        ...
        try:
            ...
            # If audio fails (e.g., unsupported codec), just run silent.
            self.audio_ready = False
...
class AssaltoRealeGame:
    ...
    def __init__(self):
        ...
        # fonts (fallback works cross-platform)
    ...
    def _run_coro(self, coro):
        # If we're already inside an asyncio loop (e.g., run_async / web),
        # schedule the task; otherwise run it to completion (desktop run()).
        try:
    ...
    async def _show_start_menu_async(self) -> None:
    ...
    async def run_async(self) -> None:
        await self._show_start_menu_async()
        ...
        while self.running:
            ...
            for ev in pygame.event.get():
                if ev.type == pygame.QUIT:
                    ...
                    self._on_click(ev.pos)  # <-- reuse the same logic; _run_coro will handle awaits
    ...
    def _on_click(self, pos: Tuple[int, int]) -> None:
        ...
        if self.placing:
            ...
            self._run_coro(self._handle_move_click((row, col)))
    ...
    async def _attempt_move(self, start: Vec2, end: Vec2) -> None:
        ...
        if captured and piece.type == "AttackPawn" and captured.type == "King":
            kr, kc = end
            ...
            if defender:
                ...
                self._record_move(start, newpos, piece, {"defense": (def_piece, defender)})
        ...
        if captured and captured.type == "ConquestPawn" and end in self.board.special_squares:
        ...
        move_cost = 1 if piece.type == "King" or (captured is None or delta == 1) else 2
...
# main.py  — web entrypoint for pygbag
import sys
import asyncio
from main_web import AssaltoRealeGame, IS_WEB  # import your game class

async def main():
    game = AssaltoRealeGame()
    if IS_WEB:
        # web: await the async loop (pygbag will call this coroutine)
        await game.run_async()
    else:
        # desktop: regular blocking loop
        game.run()

# On desktop, run immediately. On web, pygbag calls main() for us.
if __name__ == "__main__" and sys.platform not in ("emscripten", "wasi"):
    asyncio.run(main())
