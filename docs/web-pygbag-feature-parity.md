...
| Match configuration persistence | In progress: resolved setup config is stored with the match and schema-2 local save/export. | Add migrations beyond schema-1 compatibility and prove every setup combination through restore tests. | Save/load for each setup mode and resolved Random side. |
...
| Save/load | Partial: local save now writes schema 2, validates imported JSON, supports local/current export, imports JSON, restores placement state and persists undo history. Schema-1 web saves remain loadable with warnings. | Add full migration modules, legacy Python snapshot import and fixture-proven safe modal restoration. | Mid-turn, placement, timer, territory, Transform, pending-decision, schema migration and legacy import tests. |
