...
| Defended-King flow | Partial: selection UI exists but single-defender preview is still reduced. | Always show preview/confirmation and animate engine-provided bounce path. | One/multiple defender, bounce and Transform-trigger fixtures plus UI tests. |
| Timers | Partial: setup stores initial clock values; countdown is not implemented. | Add monotonic active-player clock controller and timeout victory. | Presets, untimed, placement pause, AI thinking and save/load timer tests. |
| Save/load | Partial: local save includes more setup fields but not the complete Python-equivalent snapshot. | Add versioned schema, validation, migrations, export/import and safe modal restoration. | Mid-turn, placement, timer, territory, Transform and pending-decision restore tests. |
| Victory/rematch lifecycle | Partial: game-over phase exists, but full victory modal/rematch/new-match flow is not complete. | Add victory page state, rematch same settings, restart and confirmed home return. | King, territory, timeout, rematch and new-match tests. |
