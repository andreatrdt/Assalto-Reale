...
function validateCommandRouting(
  ...
  if (command.type === "JoinMatch") {
    return matchId !== null && expectedMatchVersion === null
      ? { ok: true, value: true }
      : error("invalid_envelope", "expectedMatchVersion", "JoinMatch requires a matchId and no expected version.");
  }
...
function validateServerEvent(value: unknown): ValidationResult<ServerEvent> {
  ...
  switch (value.type) {
    case "MatchCreated":
      return typeof value.inviteCode === "string" && INVITE_PATTERN.test(value.inviteCode) && isPlayerSide(value.assignedSide) && isSnapshot(value.snapshot)
    ...
    case "RematchCreated":
      return isId(value.newMatchId) && typeof value.inviteCode === "string" && INVITE_PATTERN.test(value.inviteCode)
        ? { ok: true, value: value as ServerEvent }
