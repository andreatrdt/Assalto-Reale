...
import type { Authenticator, AuthenticatedPrincipal, Clock, IdGenerator, SeedGenerator } from "./ports.js";
import {
...
export class CommandHandler {
  ...
  async markPostGameDisconnected(principal: AuthenticatedPrincipal, matchId: string): Promise<PostGamePresenceUpdate> {
    const now = this.deps.clock.now();
    const expiresAt = new Date(now.getTime() + (this.deps.postGameReconnectGraceMs ?? 30_000));
    return this.mutatePostGamePresence(matchId, principal.playerId, (aggregate) =>
      withPostGameDisconnect(aggregate, principal.playerId, expiresAt),
    );
  }

  async expirePostGameDisconnect(principal: AuthenticatedPrincipal, matchId: string): Promise<PostGamePresenceUpdate> {
    return this.mutatePostGamePresence(matchId, principal.playerId, (aggregate) => expirePostGameGrace(aggregate, this.deps.clock.now()));
  }
  ...
  async handle(rawMessage: unknown): Promise<ServerEventEnvelope[]> {
    ...
    if (!principal) {
      return [
        this.rejection(envelope.commandId, envelope.matchId, "unauthenticated", "The command is not authenticated.", null, null, "all"),
      ];
    }
      ...
      try {
        return await this.deps.unitOfWork.run(async (tx) => {
          ...
          if (transition.changed) {
            tx.saveMatch(transition.aggregate, { kind: "expectedVersion", version: aggregate.version });
          }
          return {
            envelopes: this.envelopesFor(transition.emissions, matchId, this.deps.ids.eventId()),
  ...
  private async handleAuthenticated(envelope: ClientCommandEnvelope, playerId: string, attempt = 0): Promise<ServerEventEnvelope[]> {
    ...
    try {
      return await this.deps.unitOfWork.run(async (tx) => {
        ...
        if (existing) {
          ...
          if (envelope.matchId && (envelope.command.type === "OfferRematch" || envelope.command.type === "RespondToRematch")) {
            const current = await tx.loadMatch(envelope.matchId);
            if (current?.successorMatchId) {
              return this.announceExistingRematch(tx, current, playerId, envelope);
            }
            if (current) {
              const normalized = expirePostGameGrace(current, this.deps.clock.now());
              ...
              return [this.envelopeFor(syncEmission(normalized.aggregate, playerId), current.matchId, envelope.commandId)];
            }
          }
  ...
  private async process(tx: Transaction, envelope: ClientCommandEnvelope, playerId: string): Promise<ServerEventEnvelope[]> {
    ...
    if (command.type === "JoinMatch") {
      const aggregate = envelope.matchId ? await tx.loadMatch(envelope.matchId) : await tx.findMatchByInviteCode(command.inviteCode);
      if (!aggregate) {
        return [
          this.rejection(
            envelope.commandId,
    ...
    if (command.type === "RequestSync") {
      ...
      if (aggregate.successorMatchId) {
        ...
        const emission = successor ? rematchCreatedEmission(successor, playerId) : null;
        if (emission) {
          return [this.envelopeFor(emission, matchId, envelope.commandId)];
        }
      }
      if (isTerminalMatch(aggregate)) {
        const transition = withPostGameReentry(aggregate, playerId, this.deps.clock.now());
        if (transition.changed) {
          tx.saveMatch(transition.aggregate, { kind: "expectedVersion", version: aggregate.version });
        }
        return [
          ...this.envelopesFor(transition.emissions, matchId, envelope.commandId),
          this.envelopeFor(syncEmission(transition.aggregate, playerId), matchId, envelope.commandId),
        ];
      }
    ...
    if (command.type === "LeavePostGame") {
      ...
      if (transition.changed) {
        tx.saveMatch(transition.aggregate, { kind: "expectedVersion", version: aggregate.version });
        return this.envelopesFor(transition.emissions, matchId, envelope.commandId);
      }
      return [this.envelopeFor(syncEmission(transition.aggregate, playerId), matchId, envelope.commandId)];
    }
    ...
    if (command.type === "RespondToRematch") {
      return this.handleRespondToRematch(tx, aggregate, playerId, command.accept, envelope);
    }
  ...
  private async handleOfferRematch(
    ...
    if (!isPostGameRematchAvailable(current)) {
      ...
      return [
        ...this.envelopesFor(normalized.emissions, aggregate.matchId, envelope.commandId),
        this.rejection(
          envelope.commandId,
          aggregate.matchId,
    ...
    return this.envelopesFor(offered.emissions, aggregate.matchId, envelope.commandId);
  }
  ...
  private async handleRespondToRematch(
    ...
    if (!accept) {
      ...
      return this.envelopesFor(declined.emissions, aggregate.matchId, envelope.commandId);
    }
    return this.createRematch(tx, current, envelope);
  }

  /** Create exactly one successor rematch and persist both aggregates atomically. */
  private createRematch(tx: Transaction, previous: MatchAggregate, envelope: ClientCommandEnvelope): ServerEventEnvelope[] {
    ...
    return this.envelopesFor(creation.emissions, creation.rematch.matchId, envelope.commandId);
  }
  ...
  private async announceExistingRematch(
    ...
    const successor = aggregate.successorMatchId ? await tx.loadMatch(aggregate.successorMatchId) : null;
    const emission = successor ? rematchCreatedEmission(successor, playerId) : null;
    if (!emission) {
      return [
        this.rejection(
  ...
  private commit(
    ...
    if (!outcome.ok) {
      return [
        this.rejection(envelope.commandId, previous.matchId, outcome.code, outcome.message, outcome.version, outcome.snapshot, recipient),
      ];
    }
