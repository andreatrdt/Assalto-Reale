...
import { CommandHandler, InMemoryStore, ConcurrencyConflictError, createInMemoryPersistence, type Transaction, type UnitOfWork } from "../src/index.js";
import {
  ALICE,
  BOB,
  FixedClock,
...
function only<T extends ServerEvent["type"]>(envelopes: ServerEventEnvelope[], type: T): Extract<ServerEvent, { type: T }> {
  const found = envelopes.find((envelope) => envelope.event.type === type);
  if (!found) throw new Error(`No ${type} event in [${types(envelopes).join(", ")}]`);
  return found.event as Extract<ServerEvent, { type: T }>;
}

async function createAndJoin(): Promise<{ handler: CommandHandler; store: InMemoryStore; matchId: string; inviteCode: string }> {
  const { handler, store } = harness();
  const created = await handler.handle(message({ type: "CreateMatch", config: ONLINE_CONFIG }, { commandId: "cmd_create01", playerId: ALICE }));
  const matchCreated = only(created, "MatchCreated");
  const matchId = [...store.matches.keys()][0]!;
  await handler.handle(
    message({ type: "JoinMatch", inviteCode: matchCreated.inviteCode }, { commandId: "cmd_join0001", playerId: BOB, matchId }),
  );
...
describe("authoritative command handler (C.8.1)", () => {
  it("CreateMatch creates a server-seeded canonical match", async () => {
    ...
    const out = await handler.handle(message({ type: "CreateMatch", config: ONLINE_CONFIG }, { commandId: "cmd_create01", playerId: ALICE }));
  ...
  it("a duplicate identical command returns the same result", async () => {
    const { handler, store } = harness();
    const msg = message({ type: "CreateMatch", config: ONLINE_CONFIG }, { commandId: "cmd_dup00001", playerId: ALICE });
  ...
  it("a duplicate commandId with a different payload is rejected", async () => {
    ...
    await handler.handle(message({ type: "CreateMatch", config: ONLINE_CONFIG }, { commandId: "cmd_dup00002", playerId: ALICE }));
    const out = await handler.handle(
      message({ type: "CreateMatch", config: { ...ONLINE_CONFIG, transformEnabled: true } }, { commandId: "cmd_dup00002", playerId: ALICE }),
    );
    expect(only(out, "CommandRejected").code).toBe("duplicate_command");
  });

  it("an actor/principal mismatch is rejected", async () => {
    const { handler } = harness({ authenticator: new FixedPrincipalAuthenticator({ playerId: BOB, sessionId: "session_x1x2x3" }) });
    const out = await handler.handle(message({ type: "CreateMatch", config: ONLINE_CONFIG }, { commandId: "cmd_actor001", playerId: ALICE }));
    expect(only(out, "CommandRejected").code).toBe("unauthorized");
  });

  it("an unauthenticated command is rejected", async () => {
    const { handler } = harness({ authenticator: new FixedPrincipalAuthenticator(null) });
    const out = await handler.handle(message({ type: "CreateMatch", config: ONLINE_CONFIG }, { commandId: "cmd_noauth01", playerId: ALICE }));
    expect(only(out, "CommandRejected").code).toBe("unauthenticated");
  });

  it("a non-member command is rejected", async () => {
    const { handler, store } = harness();
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 5]]])));
    const out = await handler.handle(
      message({ type: "SubmitAction", start: [5, 5], end: [5, 6] }, { commandId: "cmd_nonmemb1", playerId: "player_carol9", matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
    expect(only(out, "CommandRejected").code).toBe("unauthorized");
  });
  ...
  it("a stale expectedMatchVersion is rejected", async () => {
    ...
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 5]]])));
    const out = await handler.handle(
      message({ type: "SubmitAction", start: [5, 5], end: [5, 6] }, { commandId: "cmd_stale001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 0 }),
    );
  ...
  it("a valid game command invokes game-core and updates canonical state", async () => {
    ...
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 5]]])));
    const out = await handler.handle(
      message({ type: "SubmitAction", start: [5, 5], end: [5, 6] }, { commandId: "cmd_move0001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
    const updated = only(out, "MatchUpdated");
    expect(updated.domainEvents.some((e) => e.type === "ActionApplied")).toBe(true);
  ...
  it("an illegal game command maps to a structured rejection and does not mutate state", async () => {
    ...
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 5]]])));
    const out = await handler.handle(
      message({ type: "SubmitAction", start: [0, 0], end: [1, 1] }, { commandId: "cmd_illegal1", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
  ...
  it("a Defended King decision flows through the server", async () => {
    const { handler, store } = harness();
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 2]], ["White", "King", [5, 3]], ["White", "DefensePawn", [4, 3]]])));
    const attack = await handler.handle(
      message({ type: "SubmitAction", start: [5, 2], end: [5, 3] }, { commandId: "cmd_dk_atk01", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
    const decision = only(attack, "DecisionRequired").decision;
    expect(decision.kind).toBe("defendedKing");
    expect(store.matches.get("match_seed01")!.state.phase).toBe("defenderSelection");

    const version = store.matches.get("match_seed01")!.version;
    const resolve = await handler.handle(
      message({ type: "ChooseDefender", position: [4, 3] }, { commandId: "cmd_dk_def01", playerId: BOB, matchId: "match_seed01", expectedMatchVersion: version }),
    );
    expect(types(resolve)).toContain("MatchUpdated");
    expect(store.matches.get("match_seed01")!.state.phase).not.toBe("defenderSelection");
  });

  it("a Transform decision flows through the server", async () => {
    const { handler, store } = harness();
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 4]]], [[5, 5]])), { transformEnabled: true });
    const move = await handler.handle(
      message({ type: "SubmitAction", start: [5, 4], end: [5, 5] }, { commandId: "cmd_tf_mv001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
    expect(only(move, "DecisionRequired").decision.kind).toBe("transform");

    const version = store.matches.get("match_seed01")!.version;
    const resolve = await handler.handle(
      message({ type: "ChooseTransform", newType: "DefensePawn" }, { commandId: "cmd_tf_ch001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: version }),
    );
    expect(types(resolve)).toContain("MatchUpdated");
    expect(store.matches.get("match_seed01")!.state.pendingTransform).toBeNull();
  });
  ...
  it("PassTurn flows through the server and changes the turn", async () => {
    ...
    const out = await handler.handle(
      message({ type: "PassTurn" }, { commandId: "cmd_pass0001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
    expect(only(out, "TurnChanged").currentPlayer).toBe("White");
    expect(store.matches.get("match_seed01")!.state.currentPlayer).toBe("White");
  });
  ...
  it("Resign ends the match", async () => {
    ...
    seedMatch(store, playingState(boardWith([["Black", "King", [10, 1]], ["White", "King", [1, 10]]])));
    const out = await handler.handle(
      message({ type: "Resign" }, { commandId: "cmd_resign01", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
  ...
  it("RequestSync returns the canonical state to the requester", async () => {
    const { handler, store } = harness();
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 5]]])), { version: 4, streamSequence: 7 });
    const out = await handler.handle(message({ type: "RequestSync", lastSeenMatchVersion: null }, { commandId: "cmd_sync0001", playerId: ALICE, matchId: "match_seed01" }));
  ...
  it("multiple events from one command receive deterministic, increasing stream sequences", async () => {
    const { handler, store } = harness();
    seedMatch(store, playingState(boardWith([["Black", "King", [10, 1]], ["White", "King", [1, 10]]])), { streamSequence: 1 });
    const out = await handler.handle(
      message({ type: "PassTurn" }, { commandId: "cmd_seq00001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 }),
    );
    ...
    expect(new Set(out.map((envelope) => envelope.matchVersion))).toEqual(new Set([2]));
  });
  ...
  it("a persistence failure does not partially save state or receipt", async () => {
    ...
    await expect(
      handler.handle(message({ type: "SubmitAction", start: [5, 5], end: [5, 6] }, { commandId: "cmd_persist1", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 })),
  ...
  it("concurrent commands against the same version cannot both commit", async () => {
    const { handler, store } = harness();
    seedMatch(store, playingState(boardWith([["Black", "AttackPawn", [5, 5]], ["Black", "King", [10, 1]], ["White", "King", [1, 10]]])));
    const [a, b] = await Promise.all([
      handler.handle(message({ type: "SubmitAction", start: [5, 5], end: [5, 6] }, { commandId: "cmd_conc0001", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 })),
      handler.handle(message({ type: "SubmitAction", start: [5, 5], end: [4, 5] }, { commandId: "cmd_conc0002", playerId: ALICE, matchId: "match_seed01", expectedMatchVersion: 1 })),
    ]);
    const outcomes = [types(a)[0], types(b)[0]].sort();
    expect(outcomes).toEqual(["CommandRejected", "MatchUpdated"]);
    const rejected = [...a, ...b].find((e) => e.event.type === "CommandRejected");
    expect((rejected!.event as Extract<ServerEvent, { type: "CommandRejected" }>).code).toBe("stale_match_version");
  ...
  it("a malformed protocol message is rejected without touching state", async () => {
    const { handler, store } = harness();
    const out = await handler.handle({ protocol: "assalto-reale", protocolVersion: 1, messageType: "command", commandId: "cmd_bad00001", sentAt: "nope", actor: {}, matchId: null, expectedMatchVersion: null, command: { type: "PassTurn" } });
  ...
  it("surfaces a ConcurrencyConflictError as a stale rejection", () => {
    expect(new ConcurrencyConflictError().name).toBe("ConcurrencyConflictError");
  });
});
