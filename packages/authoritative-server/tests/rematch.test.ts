...
import { ALICE, BOB, boardWith, harness, message, playingState, seedMatch } from "./support.js";
...
function eventFor<T extends ServerEvent["type"]>(
  ...
  const found = envelopes.find(
    (envelope) =>
      ...
      (playerId === undefined || (envelope.recipient !== "all" && envelope.recipient.playerId === playerId)),
  );
  if (!found) {
    throw new Error(`No ${type} for ${playerId ?? "any"} in [${envelopes.map((e) => e.event.type).join(", ")}]`);
  }
...
describe("authoritative rematch lifecycle", () => {
  it("rejects a rematch request for a match that has not ended", async () => {
    ...
    const out = await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer0001", playerId: ALICE, matchId: MATCH }));
  ...
  it("rejects a rematch request from a non-member", async () => {
    ...
    const out = await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer0001", playerId: CAROL, matchId: MATCH }));
  ...
  it("creates a fresh, swapped-side rematch with the same players on acceptance", async () => {
    const { handler, store } = endedMatch();
    await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer0001", playerId: ALICE, matchId: MATCH }));
    const out = await handler.handle(
      message({ type: "RespondToRematch", accept: true }, { commandId: "cmd_accept001", playerId: BOB, matchId: MATCH }),
    );
  ...
  it("declines a rematch and notifies the offerer", async () => {
    ...
    await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer0001", playerId: ALICE, matchId: MATCH }));
    const out = await handler.handle(
      message({ type: "RespondToRematch", accept: false }, { commandId: "cmd_decline01", playerId: BOB, matchId: MATCH }),
    );
    expect(eventFor(out, "RematchDeclined", ALICE).declinedByPlayerId).toBe(BOB);
  ...
  it("does not let the offerer accept their own offer", async () => {
    ...
    await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer0001", playerId: ALICE, matchId: MATCH }));
    const out = await handler.handle(
      message({ type: "RespondToRematch", accept: true }, { commandId: "cmd_accept001", playerId: ALICE, matchId: MATCH }),
    );
    expect(eventFor(out, "CommandRejected").code).toBe("unauthorized");
  });
  ...
  it("creates at most one successor under duplicate acceptance", async () => {
    ...
    const second = await handler.handle(
      message({ type: "RespondToRematch", accept: true }, { commandId: "cmd_accept002", playerId: BOB, matchId: MATCH }),
    );
  ...
  it("resolves near-simultaneous mutual offers into a single rematch", async () => {
    ...
    const out = await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer0002", playerId: BOB, matchId: MATCH }));
  ...
  it("steers a reconnecting player into the successor via RequestSync", async () => {
    ...
    const sync = await handler.handle(
      message({ type: "RequestSync", lastSeenMatchVersion: null }, { commandId: "cmd_sync00001", playerId: ALICE, matchId: MATCH }),
    );
  ...
  it("makes deliberate departure immediate, cancels an offer, and preserves the result", async () => {
    ...
    await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer_leave", playerId: ALICE, matchId: MATCH }));
    const out = await handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_leave_bob01", playerId: BOB, matchId: MATCH }));
  ...
  it("rejects rematch actions while either player is absent", async () => {
    const { handler } = endedMatch();
    await handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_leave_bob02", playerId: BOB, matchId: MATCH }));
    const out = await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_stale_offer", playerId: ALICE, matchId: MATCH }));
  ...
  it("keeps rematch availability during reconnect grace, then expires it", async () => {
    ...
    const disconnected = await h.handler.markPostGameDisconnected({ playerId: BOB, sessionId: "bob_session" }, MATCH);
    expect(eventFor(disconnected.envelopes, "PostGamePresenceChanged").presence).toBe("grace");
    const offered = await h.handler.handle(
      message({ type: "OfferRematch" }, { commandId: "cmd_grace_offer", playerId: ALICE, matchId: MATCH }),
    );
    expect(eventFor(offered, "RematchOffered").offeredByPlayerId).toBe(ALICE);
    clock.advance(30_001);
    const expired = await h.handler.expirePostGameDisconnect({ playerId: BOB, sessionId: "bob_session" }, MATCH);
    expect(eventFor(expired.envelopes, "PostGamePresenceChanged").presence).toBe("absent");
  ...
  it("refresh re-entry preserves participation but never revives an expired offer", async () => {
    const { handler, store } = endedMatch();
    await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_offer_refresh", playerId: ALICE, matchId: MATCH }));
    await handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_leave_refresh", playerId: BOB, matchId: MATCH }));
    const sync = await handler.handle(
      message({ type: "RequestSync", lastSeenMatchVersion: null }, { commandId: "cmd_sync_refresh", playerId: BOB, matchId: MATCH }),
    );
    expect(eventFor(sync, "MatchSnapshot").postGame?.presence.White).toBe("present");
    expect(store.matches.get(MATCH)?.rematchOfferedBy).toBeNull();
  });
  ...
  it("applies duplicate departures exactly once", async () => {
    const { handler, store } = endedMatch();
    const first = await handler.handle(
      message({ type: "LeavePostGame" }, { commandId: "cmd_leave_once", playerId: ALICE, matchId: MATCH }),
    );
    const version = store.matches.get(MATCH)!.version;
    const second = await handler.handle(
      message({ type: "LeavePostGame" }, { commandId: "cmd_leave_twice", playerId: ALICE, matchId: MATCH }),
    );
    expect(eventFor(first, "PostGamePresenceChanged").presence).toBe("absent");
    expect(eventFor(second, "MatchSnapshot").postGame?.presence.Black).toBe("absent");
    expect(store.matches.get(MATCH)!.version).toBe(version);
  });

  it("does not replay a stale rematch receipt after departure", async () => {
    const { handler, store } = endedMatch();
    const offer = message({ type: "OfferRematch" }, { commandId: "cmd_replay_offer", playerId: ALICE, matchId: MATCH });
    await handler.handle(offer);
    await handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_leave_replay", playerId: BOB, matchId: MATCH }));
    const replay = await handler.handle(offer);
    expect(eventFor(replay, "MatchSnapshot").postGame?.rematchOfferedBy).toBeNull();
    expect(store.matches.size).toBe(1);
  });

  it("makes Home followed by immediate return deterministic", async () => {
    const { handler } = endedMatch();
    await handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_home_now01", playerId: ALICE, matchId: MATCH }));
    const returned = await handler.handle(
      message({ type: "RequestSync", lastSeenMatchVersion: null }, { commandId: "cmd_return_now", playerId: ALICE, matchId: MATCH }),
    );
  ...
  it("converges simultaneous departures and explicit departure plus close", async () => {
    const { handler, store } = endedMatch();
    await Promise.all([
      handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_both_alice", playerId: ALICE, matchId: MATCH })),
      handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_both_bob01", playerId: BOB, matchId: MATCH })),
    ]);
    const version = store.matches.get(MATCH)!.version;
    await handler.markPostGameDisconnected({ playerId: ALICE, sessionId: "alice_session" }, MATCH);
    const aggregate = store.matches.get(MATCH)!;
  ...
  it("restores durable absence after a server restart", async () => {
    ...
    await first.handler.handle(message({ type: "LeavePostGame" }, { commandId: "cmd_restart_leave", playerId: BOB, matchId: MATCH }));
    const restarted = harness({ store: first.store });
    const rejected = await restarted.handler.handle(
      message({ type: "OfferRematch" }, { commandId: "cmd_restart_offer", playerId: ALICE, matchId: MATCH }),
    );
    expect(eventFor(rejected, "CommandRejected").code).toBe("post_game_unavailable");
  });

  it("does not roll back a successor after the creation commit boundary", async () => {
    const { handler, store } = endedMatch();
    await handler.handle(message({ type: "OfferRematch" }, { commandId: "cmd_commit_offer", playerId: ALICE, matchId: MATCH }));
    const created = await handler.handle(
      message({ type: "RespondToRematch", accept: true }, { commandId: "cmd_commit_accept", playerId: BOB, matchId: MATCH }),
    );
    const successor = eventFor(created, "RematchCreated", BOB).newMatchId;
    const lateLeave = await handler.handle(
      message({ type: "LeavePostGame" }, { commandId: "cmd_commit_leave", playerId: BOB, matchId: MATCH }),
    );
    expect(eventFor(lateLeave, "RematchCreated", BOB).newMatchId).toBe(successor);
  ...
  it("uses the same presence rules for guest and registered principals", async () => {
    ...
    expect(eventFor(guestUpdate.envelopes, "PostGamePresenceChanged").presence).toBe("grace");
    expect(eventFor(registeredUpdate.envelopes, "PostGamePresenceChanged").presence).toBe("grace");
  });
});
