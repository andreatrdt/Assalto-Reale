...
function parseMode(env: Env): RuntimeMode {
  ...
  throw new ConfigError(`NODE_ENV must be "production", "development" or "test" (got "${raw}").`);
}

function parseInteger(env: Env, key: string, fallback: number, min: number, max: number): number {
  const raw = read(env, key);
  if (raw === undefined) return fallback;
  if (!/^-?\d+$/.test(raw)) throw new ConfigError(`${key} must be an integer (got "${raw}").`);
...
function parseAuthConfig(
  env: Env,
  mode: RuntimeMode,
): Pick<RuntimeConfig, "authEnabled" | "authIssuerUrl" | "authAudience" | "authSessionIdClaim" | "authWebsocketTicketTtlMs"> {
  const authEnabled = parseBoolean(env, "AUTH_ENABLED", false);
  const ticketTtlMs = parseInteger(env, "AUTH_WEBSOCKET_TICKET_TTL_SECONDS", 60, 15, 300) * 1_000;
  if (!authEnabled) {
    return {
      authEnabled,
      authIssuerUrl: null,
  ...
  if (issuer.search || issuer.hash || (issuer.protocol !== "https:" && !(mode === "development" && issuer.protocol === "http:"))) {
    throw new ConfigError("AUTH_ISSUER_URL must be an HTTPS origin/path without query or fragment.");
  }
  ...
  if (audience.length > 512 || sessionIdClaim.length > 256) {
    throw new ConfigError("AUTH_AUDIENCE or AUTH_SESSION_ID_CLAIM is too long.");
  }
  return {
    authEnabled,
...
function parseDatabaseUrl(env: Env): string {
  ...
  if (url.protocol !== "postgres:" && url.protocol !== "postgresql:") {
    throw new ConfigError(`DATABASE_URL must use the postgres:// scheme (got "${url.protocol}").`);
  }
  return raw;
}
...
function parseOrigin(candidate: string): string {
  ...
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new ConfigError(`Allowed origin "${candidate}" must use http or https.`);
  }
  if (url.pathname !== "/" || url.search || url.hash) {
    throw new ConfigError(`Allowed origin "${candidate}" must not include a path, query or fragment.`);
  }
  return url.origin;
}
...
function parseAllowedOrigins(env: Env, mode: RuntimeMode): readonly string[] {
  ...
  if (raw === undefined) {
    if (mode === "production") {
      throw new ConfigError("MULTIPLAYER_ALLOWED_ORIGINS is required in production (comma-separated origin allowlist).");
    }
  ...
  const origins = raw
    ...
    .map(parseOrigin);
  if (origins.length === 0) {
    throw new ConfigError("MULTIPLAYER_ALLOWED_ORIGINS must list at least one origin.");
  }
...
function parseWebsocketPath(env: Env): string {
  ...
  if (!raw.startsWith("/") || /\s/.test(raw)) {
    throw new ConfigError(`WEBSOCKET_PATH must be an absolute path with no whitespace (got "${raw}").`);
  }
  return raw;
}
...
function parseSessionPath(env: Env): "/session" {
  ...
  if (raw !== undefined && raw !== "/session") {
    throw new ConfigError('SESSION_PATH is fixed at "/session" by the transport and web client; remove the override.');
  }
  return "/session";
}

function parseGuestSecret(env: Env, mode: RuntimeMode): { secret: string; usingDevelopmentSecret: boolean } {
  const raw = read(env, "GUEST_SESSION_SECRET");
  if (raw === undefined) {
    if (mode === "production") {
      throw new ConfigError(`GUEST_SESSION_SECRET is required in production and must be at least ${MIN_SECRET_BYTES} bytes.`);
    }
    return { secret: DEVELOPMENT_SECRET, usingDevelopmentSecret: true };
  }
  if (Buffer.byteLength(raw, "utf8") < MIN_SECRET_BYTES) {
    throw new ConfigError(`GUEST_SESSION_SECRET must contain at least ${MIN_SECRET_BYTES} bytes.`);
  }
  if (mode === "production" && raw === DEVELOPMENT_SECRET) {
    throw new ConfigError("GUEST_SESSION_SECRET must not be the development placeholder in production.");
  }
...
export function loadConfig(env: Env = process.env): RuntimeConfig {
  ...
  return {
    mode,
    host: read(env, "HOST") ?? (mode === "production" ? "0.0.0.0" : "127.0.0.1"),
    ...
    guestSessionTtlMs: parseInteger(env, "GUEST_SESSION_TTL_SECONDS", 12 * 60 * 60, 60, 7 * 24 * 60 * 60) * 1000,
    postGameReconnectGraceMs: parseInteger(env, "POST_GAME_RECONNECT_GRACE_SECONDS", 30, 1, 300) * 1000,
    ...auth,
    heartbeatIntervalMs: parseInteger(env, "HEARTBEAT_INTERVAL_MS", 30_000, 1_000, 300_000),
    maxPayloadBytes: parseInteger(env, "MAX_PAYLOAD_BYTES", 64 * 1024, 1024, 1024 * 1024),
    maxBufferedBytes: parseInteger(env, "MAX_BUFFERED_BYTES", 1024 * 1024, 64 * 1024, 16 * 1024 * 1024),
